...
export class Migration1716805984400 implements MigrationInterface {
    name = 'Migration1716805984400'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`users\` (\`id\` int NOT NULL AUTO_INCREMENT, \`email\` varchar(255) NOT NULL COMMENT '이메일', \`nickname\` varchar(255) NOT NULL COMMENT '닉네임', \`password\` varchar(255) NOT NULL COMMENT '비밀번호', \`name\` varchar(255) NOT NULL COMMENT '이름', \`phone_number\` varchar(255) NOT NULL COMMENT '핸드폰 번호', \`created_at\` timestamp(6) NOT NULL COMMENT '생성일자' DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` timestamp(6) NOT NULL COMMENT '수정일자' DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` timestamp(6) NULL COMMENT '삭제일자', UNIQUE INDEX \`IDX_17d1817f241f10a3dbafb169fd\` (\`phone_number\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`teams\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(255) NOT NULL COMMENT '팀명', \`logo_url\` varchar(255) NULL COMMENT '팀 로고 파일 url', \`introduce\` varchar(255) NULL COMMENT '한 줄 소개', \`since\` timestamp NOT NULL COMMENT '창단일', \`user_id\` int NOT NULL COMMENT '유저 번호', \`sports_id\` int NOT NULL COMMENT '종목 번호', \`created_at\` timestamp(6) NOT NULL COMMENT '생성일자' DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` timestamp(6) NOT NULL COMMENT '수정일자' DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` timestamp(6) NULL COMMENT '삭제일자', PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`teams\` ADD CONSTRAINT \`FK_5dd7d34e6410bba0db7bcc1e045\` FOREIGN KEY (\`user_id\`) REFERENCES \`users\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`teams\` DROP FOREIGN KEY \`FK_5dd7d34e6410bba0db7bcc1e045\``);
